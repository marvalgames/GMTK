...
namespace FIMSpace.Generating.Planning.PlannerNodes.Community
{
    public class PR_PathFindGenerateLowerCost : PR_FindPathTowards
    {
        ...
        protected override CheckerField3D CallPathfind( CheckerField3D baseChecker, CheckerField3D startChecker, CheckerField3D targetChecker, List<CheckerField3D> collisions, FieldPlanner aPlanner, FieldPlanner bPlanner )
        {
            var pathfindSetup = PathfindSetup.ToCheckerFieldPathFindParams();
            pathfindSetup.StepCostAction = CheckCost;
            return baseChecker.GeneratePathFindTowards( startChecker, targetChecker, collisions, pathfindSetup, aPlanner, bPlanner, RemoveOverlappingCells );
        }

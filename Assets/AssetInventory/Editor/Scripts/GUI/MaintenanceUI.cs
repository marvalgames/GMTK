...
namespace AssetInventory
{
    public sealed class MaintenanceUI : EditorWindow
    {
        ...
        public void OnGUI()
        {
            ...
            if (GUILayout.Button("Scan All", GUILayout.ExpandWidth(false)))
            {
                ScanAll();
            }
                            ...
                            if (GUILayout.Button("Show...", GUILayout.ExpandWidth(false)))
                            {
                                switch (validator.Type)
                                {
                                    case Validator.ValidatorType.DB:
                                        EditorUtility.DisplayDialog("Issue List", string.Join("\n", validator.DBIssues.Take(100).Select(i => $"{i.Path} ({i.Id})")), "OK");
                                        break;

                                    case Validator.ValidatorType.FileSystem:
                                        EditorUtility.DisplayDialog("Issue List", string.Join("\n", validator.FileIssues.Take(100)), "OK");
                                        break;

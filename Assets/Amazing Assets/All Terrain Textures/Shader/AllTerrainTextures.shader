Shader "Hidden/Amazing Assets/All Terrain Textures/AllTerrainTextures"
{
	Properties 
	{
	...
	{
		int splatmapChannelIndex = _Data0;
		float smoothnessFromDiffuseAlpha  = _Data1;
		float4 specular = _MapsRemapMin[0];
		...
		float4 c = tex2D(_MainTex, i.uv) + float4(lerp(GammaToLinearSpace(specular), specular, IsGammaSpace()).rgb, specular.a) * float4(1, 1, 1, diffuse.a * smoothnessFromDiffuseAlpha) * splatmap0;
	...
	float4 fragBasemapMetallic(v2f_img i) : SV_Target 
	{
		...
		metallic = lerp(GammaToLinearSpaceExact(GammaToLinearSpaceExact(metallic)), metallic, IsGammaSpace());

		float4 c = tex2D(_MainTex, i.uv) + float4(metallic, metallic, metallic, 1) * splatmap0;
	...
	float4 fragBasemapSmoothness(v2f_img i) : SV_Target 
	{
		int splatmapChannelIndex = _Data0;
		float smoothness         = _Data1;
		float readSmoothnessFromDiffuseAlpha  = _Data2;
		...
		smoothness = lerp(smoothness, diffuse.a, readSmoothnessFromDiffuseAlpha);

		float4 c = tex2D(_MainTex, i.uv) + float4(smoothness, smoothness, smoothness, 1)  * splatmap0;
	...
	{
		float rotationDegree = _Data0;
		float gammaToLinear = _Data1;
		float2 remapMinMax = _MapsRemapMin[0].xy;
		...
		//Remap
		c.r = Remap01(c.r, remapMinMax);


		if(gammaToLinear > 0.5)
	...
	float4 fragGenericHeightmapNormal(v2f_img i) : SV_Target 
	{
		...
		float3 c = HeightToNormal(i.uv, rotationDegree, remapMinMax);
	...
	float4 fragCopyChannels(v2f_img i) : SV_Target 
	{
		float4 channelIndex       = _MapsRemapMin[0];
		float replaceAlphaChannel = _Data0;
		float alphaChannelValue   = _Data1;
		float convertToLinear     = _Data2;


		float4 c = tex2D(_MainTex, i.uv);
		
		float4 target = float4(0, 0, 0, 0);
		target.r = lerp(c[channelIndex.x], GammaToLinearSpaceExact(c[channelIndex.x]), convertToLinear);
		target.g = lerp(c[channelIndex.y], GammaToLinearSpaceExact(c[channelIndex.y]), convertToLinear);
		target.b = lerp(c[channelIndex.z], GammaToLinearSpaceExact(c[channelIndex.z]), convertToLinear);
		target.a = lerp(c[channelIndex.w], GammaToLinearSpaceExact(c[channelIndex.w]), convertToLinear);

		if(replaceAlphaChannel > 0.5)
			target.a = alphaChannelValue; 
	...
	{				    
		...
		Pass	//18
	    {
			...
			CGPROGRAM
			#pragma vertex vert_img
	    	#pragma fragment fragCopyChannels
			ENDCG

...
namespace Gaia
{
    ...
    public class GaiaSessionManager : MonoBehaviour
    {
        ...
        public static GaiaSessionManager GetSessionManager(bool pickupExistingTerrain = false, bool createSession = true)
        {
            ...
            //check if still null, if yes we need to create one if creation is allowed
            if (sessionMgr == null && createSession)
            {
                //Find or create gaia Game Object
        ...
        /// <returns>True if the operation was started successfully, false if there was an issue during startup.</returns>
        public static bool CreateOrUpdateWorld(WorldCreationSettings worldCreationSettings, bool executeNow = true, bool isUpdate = false)
        {
            ...
            {
                Debug.LogWarning("In order to be able to create a multi-scene setup for terrain loading, the main scene needs to be saved to process the terrain sub-scenes correctly. Please save your scene when prompted to.");
                EditorUtility.ClearProgressBar();
                return false;
            }
            ...
            if (sessionMgr == null)
            {
                Debug.LogError("Could not find or create a Session Manager in the scene. A session manager is required for terrain creation. If you see this repeatedly when creating or updating your world, please try to save your scene BEFORE attempting to create / update the world.");
                EditorUtility.ClearProgressBar();
                return false;
            }
            ...
            {
                Debug.LogError("Could not find world creation settings to create this world. If you see this repeatedly when creating or updating your world, please try to save your scene BEFORE attempting to create / update the world.");
                EditorUtility.ClearProgressBar();
                return false;
            }
#endif
            ...
            if (!m_regenerateRun)
            {
                ...
                if (createNewTSStorage)
                {
                    ...
                    if (worldCreationSettings.m_createInScene)
                    {
                        ...
                        //position the scene view camera at half tile height, so it is likely we are within tile loading range then
                        GaiaUtils.SetSceneViewCam(new Vector3(0f, worldCreationSettings.m_tileHeight * 0.5f, -worldCreationSettings.m_tileSize * 0.5f), Vector3.zero);
                ...
                if (worldCreationSettings.m_addLoadingScreen)
                {
                    GRC_Utilities.CreateLoadingScreen();
                }
                else
                {
                    GRC_Utilities.DestroyLoadingScreen();
                }
        ...
        private static bool ExecuteClearWorld()
        {
            ...
            {
                //Skip prefabs that are not instanciated in the scene
                if (PrefabUtility.IsPartOfAnyPrefab(t.gameObject) && t.gameObject.scene.isLoaded == false)
                {
                    continue;
                }
                if (!TerrainHelper.IsWorldMapTerrain(t) && (worldDesignerInput == null || t != worldDesignerInput) && t != null)
                {
                    if (t.terrainData != null)
                    {
